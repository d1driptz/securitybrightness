# Current behavior and service reference

This document describes implemented behavior, not the roadmap. Read [Product vision](PRODUCT_VISION.md) and [Architecture](ARCHITECTURE.md) for the planned direction and trust boundaries.
...
## Internal authorization-context separation

The service/API path stores AuthorizationContext separately on SecurityEvent.authorization_context. Identity and scope evaluation prefer that immutable context exclusively, and constructing a context-backed event with protected identity fields in details is rejected. Changes to ordinary details cannot change the context's identity or granted scopes. Application credentials are never placed in this context or in desktop ReviewRequest messages.

The HTTP request and response contracts are unchanged. Audit records for context-backed requests now keep only caller data in details; authoritative application_id, application_trust, authenticated, required_scope and scope_granted remain top-level decision fields. Consumers that read duplicated identity/grant fields from audit details must use those authoritative fields instead. This intentional internal/audit migration prevents authority from being confused with caller data. Full granted-scope snapshots are no longer copied into audit details on this path.
