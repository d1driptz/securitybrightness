...
Gaps include synchronous request blocking, partial audit coverage, code-defined policy, broad scopes, no expiry/one-shot semantics, no downstream operation binding, no strong human identity or isolated Windows authority component. Storage inherits directory protections and has no rollback protection. The GUI does not yet offer security history, a scanner, assistant or device monitoring.
...
Step 2 now has a narrow implementation: the [supplied-content contract](file-security.md) analyzes immutable UTF-8 bytes for three exact private-key header markers. It returns versioned redacted locations/counts, exact-byte identity and explicit rejection/limitations without importing core authority. This is not desktop integration or full browser-rule parity. Worker availability, consumer validation and measured broader coverage remain prerequisites.

Consumer-side bounded JSON/schema validation is also implemented, including caller-owned digest/length binding and rejection of authority/source fields. It is not IPC, provenance or proof that an analyzer reported truthfully. Worker availability and broader measured coverage remain next dependencies.

The optional fixed Python worker now transports supplied bytes and validates bounded output, with one active request, deadlines and failure cleanup. It does not accept commands or execute samples. This is a tested availability boundary, not a restricted-account sandbox or protection against hostile same-user code. The [contract guide](file-security.md) records exact limits. Desktop acquisition/presentation and broader measured coverage remain unimplemented.
