...
The independent [File Security contract](file-security.md) in `file_security/text_analysis.py` analyzes supplied bounded immutable bytes with three literal private-key-header rules. It imports no core authority components and returns no authorization decision. The desktop now has a separate operator-selected regular-file acquisition/review panel; the HTTP API does not expose analysis. Tests cover the contract, worker, acquisition and real widgets. A separate [synthetic evaluation](file-analysis-evaluation.md) measures the declared worker behavior and explicitly retains known misses and benign matches. This is a narrow prototype, not broad endpoint protection.
